...
public class Accumulators {
    ...
    @SuppressWarnings("unchecked,rawtypes")
    public static <E, C extends Collection> BiConsumer<C, E> collectionNonNullAdd() { return (c, e) -> {if (e != null) c.add(e);}; }

    public static <C extends Collection<E>, E> BiConsumer<C, E> filteringAccumulator(@NonNull Predicate<E> filter, int opts) {
        return has(NON_NULL, opts)
                ? (c, e) -> { if (e != null && filter.test(e)) c.add(e); }
                : (c, e) -> { if (filter.test(e)) c.add(e); };
    }
    public static <E> BiConsumer<StringJoiner, E> filteringToStrAccumulator(@NonNull Predicate<E> filter, int opts) {
        return has(NON_NULL, opts)
    ...
    public static <E, F, C extends Collection<F>> BiConsumer<C, E> mappingAccumulator(@NonNull Function<E, F> mapping, int opts) {
        return has(NON_NULL, opts)
                ? (c, e) -> {F f = mapping.apply(e); if (f != null) c.add(f);}
                : (c, e) -> c.add(mapping.apply(e));
    }
    public static <E> BiConsumer<StringJoiner, E> mappingToStrAccumulator(@NonNull Function<E, ?> mapping, int opts) {
        return has(NON_NULL, opts)
    ...
    public static <E, F, C extends Collection<F>> BiConsumer<C, E> aFlatMappingAccumulator(@NonNull Function<E, F[]> mapping, int opts) {
        return has(NON_NULL, opts)
                ? (sj, e) -> {F[] a = mapping.apply(e); if (a != null) for (F f : a) if (f != null) sj.add(f);}
                : (sj, e) -> {F[] a = mapping.apply(e); if (a != null) for (F f : a) sj.add(f); };
    }
    public static <E> BiConsumer<StringJoiner, E> aFlatMappingToStrAccumulator(@NonNull Function<E, ? extends Object[]> mapping, int opts) {
        return has(NON_NULL, opts)
    ...
    public static abstract class SuppliedAccumulator<E, A, R> extends Accumulator<E, A, R> {
        ...
        public SuppliedAccumulator(@NonNull Supplier<A> supplier, BiConsumer<A, E> accumulator) {
            super(accumulator); this.supplier = supplier;
        }
    }
